...
func spawn_clerk(designated_room: InteriorRoom) -> ClerkUnit:
	...
	for unit in units:
		if (unit is ClerkUnit) and (unit._designated_room == designated_room):
			clerks_in_room.append(unit)
...
func spawn_agent(designated_room: InteriorRoom) -> AgentUnit:
	...
	for unit in units:
		if (unit is AgentUnit) and (unit._designated_room == designated_room):
			agents_in_room.append(unit)
